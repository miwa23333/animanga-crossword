...
function notUndefined<T>(value: T | undefined) {
  if (value === undefined) {
...
function unreachable(never: never): never {
...
function at<T>(o: { at: (index: number) => T | undefined }, index: number) {
...
function createEmptyGrid<T>(length: number) {
  return Array.from({ length }, () => Array<T | null>(length).fill(null));
}
...
function swapDirection(direction: Direction): Direction {
  switch (direction) {
    case "across":
      return "down";
    case "down":
...
function generatePuzzle(words: string[], gridSize: number) {
  const placedWords: WordInfo[] = [];
  const grid = createEmptyGrid<string>(gridSize);
  ...
  if (startDirection === "across") {
    ...
    assertType<"down">(startDirection);
        ...
        for (let j = 0; j < wordToPlace.length; ++j) {
          if (at(pWord.word, i) === at(wordToPlace, j)) {
            ...
            if (pWord.direction === "across") {
              ...
              assertType<"down">(pWord.direction);
              r = pWord.row + i;
              c = pWord.col - j;
            }
...
function findWordAt(
  ...
  return allWords.find((word) => {
    ...
    if (direction === "across") {
      ...
      assertType<"down">(direction);
      return word.col === c && r >= word.row && r < word.row + word.word.length;
    }
  });
}

function* getInputsForWord(word: WordInfo) {
  for (let i = 0; i < word.word.length; ++i) {
    ...
    const input = document.querySelector(
      `input[data-row='${r.toString()}'][data-col='${c.toString()}']`,
    );
...
interface State {
  bufferElement: HTMLInputElement;
  lastBufferText: string | undefined;
  selectedBufferInput: HTMLInputElement | undefined;
  gridElement: HTMLElement;
  placedWords: WordInfo[];
  selectedInput: HTMLInputElement | undefined;
  selectedWord: { input: HTMLInputElement; value: string }[];
...
function render({
  ...
  const unplacedWords = words.filter(
    (w) => !placedWords.some((p) => p.word === w),
  );
  ...
  gridElement.addEventListener("focusin", (e) => {
    ...
    const activeWord = findWordAt(r, c, state.currentDirection, placedWords);
    if (activeWord !== undefined) {
      state.selectedWord = Array.from(
        getInputsForWord(activeWord).map((input) => ({
          input,
          value: input.value,
        })),
      );
...
function checkAnswers(state: State) {
...
function revealAnswers(state: State) {
  const placedWords = state.placedWords;
...
function getHint(state: State) {
  ...
  const randomHint = at(
    eligibleHintCells,
    Math.floor(Math.random() * eligibleHintCells.length),
  );
...
function init({
  ...
  do {
    ...
    if (generationAttempts > 50) {
      console.warn(
        `Failed to generate a puzzle with at least ${minWords.toString()} words after 50 attempts.`,
      );
      break;
    }
...
function getElementById(document: Document, elementId: string) {
...
function querySelector(document: Document, selectors: string) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  getElementById(document, "modal-ok-btn").addEventListener(
    "click",
    hideCongratulationsModal,
  );
  querySelector(document, ".close").addEventListener(
    "click",
    hideCongratulationsModal,
  );
